...
public class ImageButton implements IUIElement
{
    ...
    public ImageButton(final String url, final int x, final int y, final Consumer<ImageButton> click) {
        this.image = ImageMaster.loadImage(url);
        this.x = (int)(Settings.scale * x);
        this.y = (int)(Settings.scale * y);
        this.w = (int)(Settings.scale * image.getWidth());
        this.h = (int)(Settings.scale * image.getHeight());
